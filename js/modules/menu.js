 ...
 */
import { getMenu, formatPrice, getCurrentUser } from './store.js';
...
export const renderMenuItems = () => {
  ...
  grid.innerHTML = items.map((item, i) => `
    <article class="menu-item-card card-hover animate-fade-in" 
      role="listitem" 
      data-id="${item.id}" 
      style="animation-delay:${i * 0.05}s"
      tabindex="0"
      aria-label="${item.name}, giá ${formatPrice(item.price)}">
      <div class="menu-item-img-wrap">
        <img class="menu-item-img" src="${item.img}" alt="${item.name}" 
          loading="lazy"
          onerror="this.src='assets/images/placeholder.svg'">
        ${!item.available ? `<div style="position:absolute;inset:0;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;"><span class="badge badge-muted">Hết món</span></div>` : ''}
      </div>
      <div class="menu-item-body">
        <div class="menu-item-meta">
          <div class="menu-item-cat">${CATEGORY_LABELS[item.category] || item.category}</div>
...
const openItemDetail = (id) => {
    ...
    modal.innerHTML = `
      <div class="modal" style="max-width:500px">
        ...
        <div class="modal-body">
          <div style="font-size:var(--font-size-xs);font-weight:600;color:var(--color-primary-500);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">${CATEGORY_LABELS[item.category]}</div>
          <h2 style="font-size:var(--font-size-2xl);font-weight:800;color:var(--color-text);margin-bottom:var(--space-2)">${item.name}</h2>
          <p style="color:var(--color-text-muted);font-size:var(--font-size-sm);line-height:1.7;margin-bottom:var(--space-5)">${item.desc}</p>
          ...
          <div class="form-group">
            ...
            <input class="form-control" type="text" id="item-note" placeholder="Ít cay, không hành..." value="${noteVal}" maxlength="100">
          </div>
          <div style="display:flex;gap:var(--space-3)">
            <button class="btn btn-outline" id="detail-cancel" style="flex:0 0 auto">Quay lại</button>
            <button class="btn btn-primary btn-block" id="detail-add-cart" ${!item.available ? 'disabled' : ''}>
  ...
  const updateQty = () => {
    ...
    modal.querySelector('#detail-add-cart').textContent = `🛒 Thêm vào giỏ — ${formatPrice(item.price * qty)}`;
  };
