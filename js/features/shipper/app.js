import { getOrders, saveOrders, addPoints, calculateOrderPoints, formatDate, formatPrice } from '../../data/store.js';
...
const showLogin = () => {
  ...
  document.getElementById('shipper-login-form')?.addEventListener('submit', (event) => {
    ...
    const result = loginShipper(phone, password);
    if (!result.ok) {
...
const bindDetail = () => {
  ...
  document.querySelectorAll('[data-shipper-status]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      if (!ok) return;
      const updated = updateOrderStatus(selectedOrderId, nextStatus);
      if (!updated) {
