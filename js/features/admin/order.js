...
import { getOrders, saveOrders, addPoints, calculateOrderPoints, formatPrice, formatDate } from '../../data/store.js';
...
const renderSelectedOrderDetail = () => {
  ...
  detailEl.querySelectorAll('[data-set-status]')?.forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      if (!ok) return;
      const updated = updateOrderStatus(selectedOrderId, next);
      if (!updated) {
